...
namespace AutoTDSYouTube
{
    public partial class Youtube : Form
    {
        ...
        private async Task WorkSubscribeYoutube()
        {
            ...
            querys[0].Click();
            delay(2);
            querys = driver.FindElements(By.XPath("//*[@id=\"home\"]/li[4]/a"));
            querys[0].Click();
            delay(2);
            querys = driver.FindElements(By.XPath("//*[@id=\"youtube-coin\"]/li[1]/a"));
            querys[0].Click();
            delay(2);
            ...
            {
                string linkYtb = lisNV[i].GetAttribute("title");
                item.Click();
                dataGridView1.Invoke(new Action(() =>
                {
                    dataGridView1.Rows.Add(TongJob + 1, "Subscribe Youtube", linkYtb, "Mở Link");
                    dataGridView1.CurrentCell = dataGridView1.Rows[TongJob].Cells[2];
                }));
                delay(20);
                var tabs = driver.WindowHandles;
                driver.SwitchTo().Window(tabs[1]);
                var sbubt = driver.FindElements(By.XPath("//*[@id=\"app\"]/div[1]/ytm-watch/ytm-single-column-watch-next-results-renderer/div/ytm-slim-video-metadata-section-renderer/ytm-slim-owner-renderer/div/ytm-subscribe-button-renderer/div/div/button"));
        ...
        private async Task WorkCommentYoutube()
        {
            int j = 0;
            var query = driver.FindElements(By.XPath("//*[@id=\"top\"]/div/div[1]/nav/button"));
            query[0].Click();
            delay(2);
            query = driver.FindElements(By.XPath("//*[@id=\"home\"]/li[4]/a"));
            query[0].Click();
            delay(2);
            query = driver.FindElements(By.XPath("//*[@id=\"youtube-coin\"]/li[2]/a"));
            query[0].Click();
            delay(2);
            var lisNV = driver.FindElement(By.Id("load")).FindElements(By.TagName("button"));
            ...
            for (int i = 0; i < lisNV.Count; i++)
            {
                ...
                dataGridView1.Invoke(new Action(() =>
                {
                    dataGridView1.Rows.Add(TongJob + 1, "Comment Youtube", linkYtb, "Xử lý nội dung");
                    dataGridView1.CurrentCell = dataGridView1.Rows[TongJob].Cells[2];
                }));
                ...
                delay(5);
                //Subscribe
                var commentbt = driver.FindElements(By.XPath("//*[@id='app']/div[1]/ytm-watch/ytm-single-column-watch-next-results-renderer/div/ytm-item-section-renderer/lazy-list/ytm-comments-entry-point-header-renderer/button"));
                ...
                {
                    dataGridView1.Rows[TongJob].Cells[3].Value = "click thẻ comment";
                    commentbt[0].Click();
                    delay(4);
